...
def get_predictions(root_path, model_path, batch_size=8, nproc: int = -1):
    """Get predictions for all images in a directory. Use cache if available.

    Args:
        root_path (_type_): Path to directory containing images.
        model_path (_type_): Path to model file.
    ...
    if os.path.exists(root_path + '/prediction_cache.h5'):
        print('Loading predictions from cache')
        with h5.File(root_path + '/prediction_cache.h5', 'r') as f:
    ...
    if new_paths:
        ...
        predictor = Predictor(model_path=model_path, root_path=root_path, img_paths=new_paths, nproc=nproc)
        ...
        print('Saving cache...')
        with h5.File(root_path + '/prediction_cache.h5', 'w') as f:
...
def load_and_process_image(image, target_width, target_height):
    """
...
def worker(x):
    ...
    """
    q, path = x
    img = load_and_process_image(path, 512, 512)
    if img is not None:
        q.put((path, img))

class DataLoader:
    def __init__(self, root_path=None, img_paths=None, nproc: int = 8, maximum_look_ahead: int = 128):
        ...
        self.generator = self.image_generator(self.nproc, self.maximum_look_ahead)

    def __call__(self):
        return self.generator

    def image_generator(self, nproc, maximum_look_ahead: int = 128):
        """Multi-threaded image dataloader for deepdanbooru. All necessary preprocesses are done in this function.
...
class Predictor:
    def __init__(self, model_path, root_path=None, img_paths=None, nproc: int = -1):
    ...
    # @staticmethod
    # def make_batch(gen: Generator, batchsize: int = 16):
    #     for item in gen:
    #         batch = [item]
    #         for item, _ in zip(gen, range(batchsize - 1)):
    #             batch.append(item)
    #         yield batch
    ...
    def predict(self, batch_size: int = 8) -> Tuple[List[Path], np.ndarray]:
        ...
        self.dataloader = DataLoader(root_path=self.root_path, img_paths=self.img_paths, nproc=self.nproc)
        for device in gpu_devices:
            tf.config.experimental.set_memory_growth(device, True)
        ...
        def gen():
            for path, arr in self.dataloader():
                paths.append(path)
                yield arr
        ds = tf.data.Dataset.from_generator(gen, tf.float32, tf.TensorShape([512, 512, 3]))
        # specify dataset size for display in progress bar
        print("Initializing...")
        # pbar = tqdm(total=len(self.img_paths), desc="Predicting", unit="image")
